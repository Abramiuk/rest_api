...
def _running_in_container():
    ...
    try:
        with open("/proc/1/cgroup", "rt", errors="ignore") as handle:
            content = handle.read()
...
if TEST_DATABASE_URL is None:
    if _running_in_container():
        TEST_DATABASE_URL = "mongodb://db:27017/library4_test"
    else:
        TEST_DATABASE_URL = "mongodb://localhost:27018/library4_test"
...
def test_database_container(docker_compose_file):
    ...
    if command is None or shutil.which(command[0]) is None:
        # Running inside a container without a usable Docker CLI.
        yield
        return
